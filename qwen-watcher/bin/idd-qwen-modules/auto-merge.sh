#!/usr/bin/env bash
# ─── Auto-merge Processor (#99) ───────────────────────────────────────────────
# 実装 PR（head `^codex/issue-.*-impl`）を対象に、GitHub ネイティブ auto-merge を
# 有効化する（`gh pr merge --auto --squash --delete-branch`）。watcher は **直接
# merge せず**、実 merge は GitHub が必須 check 全 green 到達時に squash 実行する。
#
# 完全自動化 kill switch `FULL_AUTO_ENABLED` と `AUTO_MERGE_ENABLED` の **AND 二重
# opt-in** で動き、いずれか OFF（既定）では外部副作用ゼロで no-op。
#
# 依存: core_utils.sh（logger / idd_secure_mktemp）
# 設定: AUTO_MERGE_ENABLED（true のみ有効）
#       AUTO_MERGE_MAX_PRS（上限数）
#       AUTO_MERGE_GIT_TIMEOUT（gh/git タイムアウト秒）
#       AUTO_MERGE_HEAD_PATTERN（head branch pattern）
...
am_log()  { _qw_log_info "auto-merge" "$*"; }
am_warn() { _qw_log_warn "auto-merge" "$*"; }
am_error(){ _qw_log_error "auto-merge" "$*"; }
...
# ─── am_should_enable_for_pr: 1 PR が auto-merge 有効化の対象かを判定 ─────────
...
am_should_enable_for_pr() {
  ...
  has_ready=$(printf '%s' "$pr_json" | jq -r --arg l "$LABEL_READY" \
  ...
  # MERGEABLE 以外（CONFLICTING / UNKNOWN）は触らない（merge-queue へ委譲）
...
# ─── am_enable_auto_merge_for_pr: 1 PR に GitHub native auto-merge を有効化 ──
...
#   gate: full_auto_enabled() AND am_resolve_gate_enabled の AND 二重 opt-in
process_auto_merge() {
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
  ...
  am_log "=== auto-merge.sh テスト ==="
  am_log "AUTO_MERGE_ENABLED=$AUTO_MERGE_ENABLED"
  am_log "AUTO_MERGE_MAX_PRS=$AUTO_MERGE_MAX_PRS"
  am_log "AUTO_MERGE_HEAD_PATTERN=$AUTO_MERGE_HEAD_PATTERN"

  # 関数定義の確認
  declare -f process_auto_merge >/dev/null 2>&1 && am_log "process_auto_merge: OK" || am_log "process_auto_merge: MISSING"
  declare -f am_resolve_gate_enabled >/dev/null 2>&1 && am_log "am_resolve_gate_enabled: OK" || am_log "am_resolve_gate_enabled: MISSING"
  declare -f am_should_enable_for_pr >/dev/null 2>&1 && am_log "am_should_enable_for_pr: OK" || am_log "am_should_enable_for_pr: MISSING"
  declare -f am_enable_auto_merge_for_pr >/dev/null 2>&1 && am_log "am_enable_auto_merge_for_pr: OK" || am_log "am_enable_auto_merge_for_pr: MISSING"
fi