...
- **`idd-qwen-issue-watcher.sh` 変更時**:
  - dry run: `REPO=owner/test REPO_DIR=/tmp/test-repo ~/bin/idd-qwen-issue-watcher.sh` を対象なし状態で流し、`処理対象の Issue なし` で正常終了すること
...
- **`qwen-watcher/bin/idd-qwen-issue-watcher.sh` の変更**: 既稼働の cron / launchd を壊さない
...
- Watcher 実装: `qwen-watcher/bin/idd-qwen-issue-watcher.sh`
